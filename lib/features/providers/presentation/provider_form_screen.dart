...
class _ProviderFormScreenState extends ConsumerState<ProviderFormScreen> {
  ...
  Future<void> _loadExisting() async {
    ...
    final provider = await repo.getById(int.parse(widget.id!));
    if (provider == null || !mounted) return;
    final creds = repo.decodeCredentials(provider);
    setState(() {
      _selectedType = provider.type;
      _nameCtrl.text = provider.name;
      _isDefault = provider.isDefault;
      _credCtrls.clear();
      final meta = GatewayRegistry.metadata[provider.type];
      if (meta != null) {
        for (final f in meta.credentialFields) {
          _credCtrls[f.key] = TextEditingController(
            text: creds[f.key]?.toString() ?? '',
          );
        }
      }
      _loaded = true;
  ...
  Widget build(BuildContext context) {
    ...
    // Show loading while fetching provider data in edit mode
    if (isEdit && !_loaded) {
      return Scaffold(
  ...
  Future<void> _save() async {
    ...
    try {
      final repo = ref.read(providerRepositoryProvider);
      final creds = Map<String, dynamic>.fromEntries(
        _credCtrls.entries.map((e) => MapEntry(e.key, e.value.text.trim())),
      );
      final channels = _meta?.channels ?? [];
      ...
      if (isEdit) {
        ...
        if (provider != null) {
          ...
          if (_isDefault) await repo.setDefault(provider.id);
        }
      } else {
        final provider = await repo.add(
          name: _nameCtrl.text.trim(),
          type: _selectedType!,
          credentials: creds,
          channels: channels,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(isEdit ? 'Provedor atualizado com sucesso!' : 'Provedor salvo com sucesso!'),
